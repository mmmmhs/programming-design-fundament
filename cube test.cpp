...
int main(){
	...
	for(int r = 0;r < M;r ++){
		...
		int n = rand() % 20 + 1;
		void(*a[12])() = {U, Ui, L, Li, R, Ri, B, Bi, D, Di, F, Fi};
		while(n--)
			a[rand() % 12](); 
		ping();
	}
...
void hand(int dir, int times)		// dir = 1 ~ 4Ϊ���� 5 ~ 8Ϊ���֣������濪ʼ˳ʱ����ת�� 
{
	if(dir == 1)
	{
...
void midl(int dir)					// dir = 1 ~ 4Ϊ������� 5 ~ 8Ϊ������ң������濪ʼ˳ʱ����ת�� 
{
	if(dir == 1)
	{
		hand(1, 1);
...
void xiaoyv(int dir)				//dir = 1 Ϊ��С�㣬 dir = 2 Ϊ��С�㡣 
{
	if(dir == 1)
	{
		Li();
...
void step3()			//�ؽǹ�λ 
{
	...
	{
		while(*jf1 == bottom || *jl1 == bottom || *jb1 == bottom || *jr1 == bottom || *jf2 == bottom || *jl2 == bottom || *jb2 == bottom || *jr2 == bottom)		//����������һ��û�е׿� 
		{
			U();
			if(*jf1 == bottom)
			{
...
void out(){
	for(int i = 0;i < 6;i ++){
		for(int k = 1;k < 9;k ++){
			if(cube[i][k / 3][k % 3] != cube[i][(k - 1) / 3][(k - 1) % 3])
			{
				cout << 0;
				return;
			}				
